...
```markdown
### امنیت و پیکربندی
در این پروژه، امنیت به عنوان یک اولویت در نظر گرفته شده است:

*   **مدیریت اطلاعات حساس:** تمام تنظیمات حساس (مانند `SECRET_KEY` و دیتابیس) از فایل `settings.py` جدا شده و از طریق فایل `.env` مدیریت می‌شوند. این فایل در `.gitignore` قرار دارد تا از انتشار عمومی جلوگیری شود.
*   **حفاظت در برابر حملات:** استفاده از Middlewareهای استاندارد جنگو برای جلوگیری از حملات **CSRF**، **XSS** و **SQL Injection**.
*   **امنیت احراز هویت:** پیاده‌سازی مدیریت نشست‌ها (Session Management) به صورت امن و هش کردن پسوردها با الگوریتم‌های استاندارد.
*   **جداسازی محیط‌ها:** ارائه فایل `.env.example` برای راهنمایی توسعه‌دهندگان دیگر بدون لو رفتن کلیدهای اصلی.
